...
const ShowSlice = createSlice({
  name: "counter",
  initialState: { //action ko jagha di mene initialstate main
    data:[],
    load:false
  },
  reducers: { //2 reducer bnaye h 
    setData: (state,action) => {  //ye reducer bnaya ye data fatch krne k liye
        state.data = action.payload
        state.load = false
       
    },
    loading:(state)=>{      //ye reducer loading krne k liye
      state.load = true
    },
  },
});
